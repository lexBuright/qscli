...
class NativeTimeSeriesStore(object):
    ...
    @classmethod
    def get_last_values(cls, metric_data, num, ident=None, id_series=None, ident_period=1, index=0):
        ...
        has_ids = cls.get_has_ids(metric_data)

        negative_index = -1 - index

        if has_ids:
            if ident is None:
                id_entries = sorted(cls._get_values(metric_data), key=lambda x: x.get('id'))
                entries = id_entries[negative_index:negative_index - num:-1]
            else:
                before_id_entries = sorted([x for x in cls._get_values(metric_data) if x.get('id') <= ident], key=lambda x: x.get('id'))
                entries = before_id_entries[negative_index:negative_index - num:-1]
        else:
            if ident is not None:
                raise ValueError(ident)
            else:
                entries = cls._get_values(metric_data)[negative_index:negative_index - num:-1]
